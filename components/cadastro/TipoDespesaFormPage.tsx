...
import { tipoDespesaSchema, type TipoDespesaInput } from '@/lib/validators/tipo-despesa.schema'
import type { TipoDespesa, TipoDespesaListItem, PlanoContasListItem } from '@/types/cadastros.types'

interface Props { tipo?: TipoDespesa }
...
function Check({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer', color: 'var(--texto-principal)', userSelect: 'none' }}>
      <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} style={{ cursor: 'pointer', width: 13, height: 13 }} />
      {label}
    </label>
  )
}
...
export default function TipoDespesaFormPage({ tipo }: Props) {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<TipoDespesaInput>({
    resolver: zodResolver(tipoDespesaSchema),
    defaultValues: { natureza: 'A', ind_pis_cofins: false, ind_imposto: false, ind_capex: false, ativo: true },
  })

  const indImposto = watch('ind_imposto')
  ...
  useEffect(() => {
    ...
    reset({
      ...
      ind_imposto:    tipo.ind_imposto,
      tipo_imposto:   (tipo.tipo_imposto as TipoDespesaInput['tipo_imposto']) ?? undefined,
      ind_capex:      tipo.ind_capex,
      pai_id:         tipo.pai_id ?? undefined,
      ativo:          tipo.ativo,
    })
    if (tipo.pai_id) {
      fetch(`/api/cadastro/tipos-despesa/${tipo.pai_id}`)
        .then(r => r.ok ? r.json() : null)
        .then(d => d && setPaiCodigo(d.codigo ?? ''))
    }
  }, [tipo, reset])

  async function onSubmit(data: TipoDespesaInput) {
    setSaving(true)
    try {
      const url    = tipo ? `/api/cadastro/tipos-despesa/${tipo.id}` : '/api/cadastro/tipos-despesa'
      ...
      toast.success(tipo ? 'Tipo de despesa atualizado!' : 'Tipo de despesa cadastrado!')
      if (!tipo) router.push(`/cadastro/tipos-despesa/${json.id}`)
      else router.refresh()
  ...
  async function desativar() {
    ...
    setDeleting(true)
    try {
      await fetch(`/api/cadastro/tipos-despesa/${tipo.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ativo: false }),
      })
      toast.success('Tipo de despesa desativado')
      router.push('/cadastro/tipos-despesa')
  ...
  async function excluir() {
    ...
    setExcluding(true)
    try {
      const res = await fetch(`/api/cadastro/tipos-despesa/${tipo.id}`, { method: 'DELETE' })
  ...
  function btnLimpar(onClear: () => void) {
    return (
  ...
  return (
    <>
      {modalPai && (
        <PickerModal<TipoDespesaListItem>
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', backgroundColor: 'var(--bg-card)', borderBottom: '1px solid var(--borda-media)', position: 'sticky', top: 0, zIndex: 20 }}>
          <button type="button" onClick={() => router.push('/cadastro/tipos-despesa')}
          ...
          <button type="button" onClick={() => router.push('/cadastro/tipos-despesa/novo')}
            style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: 'none', border: '1px solid var(--borda-media)', borderRadius: 3, fontSize: 12, cursor: 'pointer', color: 'var(--texto-secundario)' }}>
            <Plus size={13} /> Novo
          </button>
        ...
        <div style={{ padding: '14px 20px', display: 'flex', gap: 16, overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {/* Natureza */}
            <Row label="Natureza:">
              <Select {...register('natureza')} style={{ width: 220 }}>
                <option value="A">Administrativa</option>
                <option value="F">Financeira</option>
                <option value="I">Imposto / Tributo</option>
              </Select>
            </Row>

            {/* Tipo de Imposto — visível só quando ind_imposto = true */}
            {indImposto && (
              <Row label="Tipo de Imposto:">
                <Select {...register('tipo_imposto')} style={{ width: 200 }}>
            ...
            <fieldset style={{ border: '1px solid var(--borda-media)', borderRadius: 4, padding: '8px 14px 12px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 6 }}>
                <Check
                  ...
                  onChange={v => setValue('ind_pis_cofins', v)}
                />
                <Check
                  label="É Imposto / Tributo"
                  checked={watch('ind_imposto')}
                  onChange={v => { setValue('ind_imposto', v); if (!v) setValue('tipo_imposto', undefined) }}
                />
                <Check
                  label="CAPEX (Investimento)"
                  checked={watch('ind_capex')}
                  onChange={v => setValue('ind_capex', v)}
                />
              </div>
            </fieldset>
          </div>
