...
interface Props { despesa?: Despesa }
...
export default function DespesaFormPage({ despesa }: Props) {
  ...
  const [rateioIdx,        setRateioIdx]        = useState<number | null>(null)  // linha destacada
  const [rateioEdicao,     setRateioEdicao]     = useState(false)               // modo edição ativo
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<DespesaInput>({
  ...
  const dataDespesa  = watch('data_despesa')
  const valorWatch   = watch('valor')
  // calculadoras de rateio — no nível do componente para acesso estável nos handlers
  ...
  function gerarParcelas() {
    ...
    const base     = geraVenc1 || dataDespesa
  ...
  useEffect(() => {
    if (!despesa) return
    setValue('pessoa_id',         despesa.pessoa_id)
    setValue('tipo_despesa_id',   despesa.tipo_despesa_id)
    setValue('cod_tipo_cobranca', despesa.cod_tipo_cobranca ?? undefined)
    setValue('centro_custo_id',   despesa.centro_custo_id   ?? undefined)
    setValue('conta_banco_id',    despesa.conta_banco_id    ?? undefined)
    setValue('ind_avista',         despesa.ind_avista)
    setValue('destino',            despesa.destino ?? undefined)
    setValue('data_despesa',       despesa.data_despesa)
    setValue('data_competencia',   despesa.data_competencia ?? '')
    setValue('data_pagamento',     despesa.data_pagamento   ?? '')
    setValue('documento',          despesa.documento        ?? '')
    setValue('valor',              parseFloat(despesa.valor))
    setValue('num_parcelas',       despesa.num_parcelas)
    setValue('intervalo_dias',     despesa.intervalo_dias)
    ...
    // inicializa campos de geração automática com valores salvos
    setGeraVenc1(despesa.data_despesa)
    setGeraQtd(String(despesa.num_parcelas))
    setGeraIntervalo(String(despesa.intervalo_dias))
    setGeraValor(parseFloat(despesa.valor) || 0)

    // inicializa parcelas e rateios salvos
    if (despesa.parcelas?.length) {
      setParcelas(despesa.parcelas.map(p => ({
    ...
    if (despesa.rateios?.length) {
      setRateios(despesa.rateios.map(r => ({
  ...
  async function onSubmit(data: DespesaInput) {
    ...
    // Tipo Administrativo exige ao menos um rateio
    if (tipoDespesaNatureza === 'A' && rateios.length === 0) {
      toast.error('Tipo de despesa Administrativa requer pelo menos um rateio de centro de custo.')
      setAba('Rateio')
      return
    }
    ...
    setSaving(true)
    try {
      const url    = despesa ? `/api/financeiro/despesas/${despesa.id}` : '/api/financeiro/despesas'
      ...
      console.log('[onSubmit] rateios no payload:', JSON.stringify(payload.rateios))
      ...
      if (!despesa) router.push(`/financeiro/despesas/${json.id}`)
      else router.refresh()
    } finally { setSaving(false) }
  }

  async function excluir() {
    if (!despesa || !confirm('Excluir esta despesa?')) return
    setDeleting(true)
    try {
      const res = await fetch(`/api/financeiro/despesas/${despesa.id}`, { method: 'DELETE' })
      if (!res.ok) {
...
{picker === 'conta_banco' && (
      ...
      <form onSubmit={handleSubmit(onSubmit, (errs) => {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderBottom: '1px solid var(--borda-suave)', flexShrink: 0 }}>
          <button type="button" onClick={() => router.push('/financeiro/despesas')}
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--texto-principal)', marginLeft: 4 }}>
            {despesa ? `Despesa #${despesa.id}` : 'Nova Despesa'}
          </span>
          <div style={{ flex: 1 }} />
          {despesa && (
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--borda-suave)', paddingLeft: 16, flexShrink: 0 }}>
          {ABAS.map(a => (
            <button key={a} type="button" onClick={() => setAba(a)}
              ...
              {a}{a === 'Parcelas' && numParcelas > 1 ? ` (${numParcelas}x)` : ''}{a === 'Rateio' && tipoDespesaNatureza === 'A' && rateios.length === 0 ? ' ⚠' : ''}
            </button>
          ))}
        </div>
        ...
        <fieldset className="form-fieldset">
          ...
          {/* Datas */}
          <Row label="Data Despesa:*" error={errors.data_despesa?.message}>
            <input type="date" {...register('data_despesa')}
              style={{ padding: '3px 6px', backgroundColor: 'var(--bg-input)', color: 'var(--texto-principal)', border: errors.data_despesa ? '1px solid var(--cor-erro)' : '1px solid var(--borda-media)', borderRadius: 3, fontSize: 12, outline: 'none' }} />
          </Row>
          ...
          <fieldset className="form-fieldset" style={{ margin: 0 }}>
            ...
            {watch('destino') === 'B' ? (
              ...
              <p style={{ margin: 0, fontSize: 12, color: 'var(--texto-terciario)' }}>Marque para registrar pagamento via banco</p>
            )}
          </fieldset>
          ...
          <fieldset className="form-fieldset" style={{ margin: 0 }}>
            ...
            {watch('destino') === 'C' ? (
              <p style={{ margin: 0, fontSize: 12, color: 'var(--texto-secundario)' }}>Pagamento registrado no caixa da empresa</p>
            ) : (
              <p style={{ margin: 0, fontSize: 12, color: 'var(--texto-terciario)' }}>Marque para registrar pagamento via caixa</p>
            )}
          </fieldset>
        ...
        {aba === 'Rateio' && (() => {
          ...
          return (
            <>
              {tipoDespesaNatureza === 'A' && rateios.length === 0 && (
                <div style={{ padding: '8px 12px', marginBottom: 4, backgroundColor: 'var(--cor-aviso)18', border: '1px solid var(--cor-aviso)60', borderRadius: 4, fontSize: 12, color: 'var(--cor-aviso)', fontWeight: 500 }}>
                  ⚠ Tipo de despesa <strong>Administrativa</strong> — é obrigatório informar ao menos um rateio de centro de custo antes de salvar.
                </div>
              )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <input readOnly value={rateioCentroNome} onClick={() => setPicker('centro_custo_rateio' as PickerKey)}
                  placeholder="Clique para selecionar..." style={{ ...inStyle, flex: 1, cursor: 'pointer' }} />
                <button type="button" onClick={() => setPicker('centro_custo_rateio' as PickerKey)}
