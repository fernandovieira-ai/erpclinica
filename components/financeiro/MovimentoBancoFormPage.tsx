...
interface Props { movimento?: MovimentoBanco }
...
type PickerKey = 'conta_banco' | 'pessoa' | 'tipo_operacao'
...
export default function MovimentoBancoFormPage({ movimento }: Props) {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<MovimentoBancoInput>({
    resolver: zodResolver(movimentoBancoSchema),
    defaultValues: {
      tipo:           'S',
  ...
  useEffect(() => {
    if (!movimento) return
    setValue('conta_banco_id',   movimento.conta_banco_id)
    ...
    setValue('data_predatado',   movimento.data_predatado    ?? '')
    setValue('data_referencia',  movimento.data_referencia   ?? '')
    ...
    setNomeContaBanco(movimento.conta_banco_desc   ?? '')
    setNomePessoa(movimento.pessoa_nome            ?? '')
    setNomeTipoOperacao(movimento.tipo_operacao_desc ?? '')
  }, [movimento, setValue])

  async function onSubmit(data: MovimentoBancoInput) {
    setSaving(true)
    try {
      const url    = movimento ? `/api/financeiro/movimento-banco/${movimento.id}` : '/api/financeiro/movimento-banco'
      ...
      if (!movimento) router.push(`/financeiro/movimento-banco/${json.id}`)
      else router.refresh()
  ...
  async function conciliar() {
    ...
    try {
      ...
      const res = await fetch(`/api/financeiro/movimento-banco/${movimento.id}`, {
        method: 'PATCH',
  ...
  async function excluir() {
    if (!movimento || !confirm('Excluir este movimento bancário?')) return
    setDeleting(true)
    try {
      const res = await fetch(`/api/financeiro/movimento-banco/${movimento.id}`, { method: 'DELETE' })
      if (!res.ok) { toast.error('Erro ao excluir'); return }
      toast.success('Movimento excluído!')
      router.push('/financeiro/movimento-banco')
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderBottom: '1px solid var(--borda-suave)', flexShrink: 0 }}>
          <button type="button" onClick={() => router.push('/financeiro/movimento-banco')}
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--texto-principal)', marginLeft: 4 }}>
            {movimento ? `Movimento Bancário #${movimento.id}` : 'Novo Movimento Bancário'}
          </span>
          {movimento && (
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {aba === 'Dados' && (<>
            ...
            <LookupField
              ...
              onClear={() => { setValue('conta_banco_id', 0); setNomeContaBanco('') }}
              error={errors.conta_banco_id?.message}
            />
            ...
            <Row label="Data Pré-datado:">
              <input type="date" {...register('data_predatado')}
          ...
          {aba === 'Complemento' && (<>
            ...
            {movimento && isConciliado && (<>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: '4px 6px' }}>
                ...
                <span style={{ textAlign: 'right', fontSize: 11, color: 'var(--texto-terciario)' }}>Conciliado por:</span>
                <span style={{ fontSize: 11, color: 'var(--texto-secundario)' }}>{movimento.conciliado_por ?? '—'}</span>
              </div>
            </>)}
