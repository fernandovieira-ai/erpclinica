...
import type { TituloPagarListItem, TituloPagarListResponse } from '@/types/cadastros.types'
...
export default function TitulosPagarPage() {
  ...
  const [dados,      setDados]      = useState<TituloPagarListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/financeiro/titulos-pagar?${sp}`)
      if (!res.ok) { toast.error('Erro ao carregar'); return }
      const data: TituloPagarListResponse = await res.json()
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1 className="page-title">Títulos a Pagar</h1>
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>Controle de contas e títulos a pagar</div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/financeiro/titulos-pagar/novo')}>
            ...
            <table className="table-base">
              <thead>
                <tr>
                  ...
                  <th style={{ width: 100 }}>Liquidação</th>
                  <th style={{ width: 130, textAlign: 'right' }}>Valor Original</th>
                  <th style={{ width: 130, textAlign: 'right' }}>Valor Pago</th>
              ...
              <tbody>
                ...
                {dados.map(t => {
                  ...
                  return (
                    <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => router.push(`/financeiro/titulos-pagar/${t.id}`)}>
                      ...
                      <td style={{ color: 'var(--texto-secundario)' }}>{t.tipo_despesa_desc || '—'}</td>
