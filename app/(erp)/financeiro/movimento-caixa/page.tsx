...
import type { MovimentoCaixaListItem, MovimentoCaixaListResponse } from '@/types/cadastros.types'
...
function OrigemBadge({ m }: { m: MovimentoCaixaListItem }) {
...
export default function MovimentoCaixaPage() {
  ...
  const [dados,      setDados]      = useState<MovimentoCaixaListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/financeiro/movimento-caixa?${sp}`)
      if (!res.ok) { toast.error('Erro ao carregar'); return }
      const data: MovimentoCaixaListResponse = await res.json()
      setDados(data.dados); setTotal(data.total); setPages(data.pages)
    } finally { setLoading(false) }
  }, [busca, tipo, conciliado, dataInicio, dataFim, page])

  useEffect(() => { carregar() }, [carregar])
  useEffect(() => { setPage(1) }, [busca, tipo, conciliado, dataInicio, dataFim])
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1 className="page-title">Movimento Caixa</h1>
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>Extrato de movimentações de caixa</div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/financeiro/movimento-caixa/novo')}>
        ...
        <div className="card">
          <div className="table-wrapper">
            <table className="table-base">
              ...
              <tbody>
                {loading && (
                  <tr><td colSpan={9} style={{ textAlign: 'center', padding: 24, color: 'var(--texto-terciario)', fontSize: 13 }}>Carregando...</td></tr>
                )}
                {!loading && dados.length === 0 && (
                  <tr><td colSpan={9} style={{ textAlign: 'center', padding: 24, color: 'var(--texto-terciario)', fontSize: 13 }}>Nenhum movimento encontrado</td></tr>
                )}
                {dados.map(m => {
                  const tp = TIPO_LABEL[m.tipo] ?? { label: m.tipo, cor: 'inherit' }
                  return (
                    <tr key={m.id} style={{ cursor: 'pointer' }} onClick={() => router.push(`/financeiro/movimento-caixa/${m.id}`)}>
