...
import type { TipoReceitaListItem, TipoReceitaListResponse } from '@/types/cadastros.types'

const NATUREZA_LABEL: Record<string, string> = { O: 'Operacional', F: 'Financeira', E: 'Eventual' }

export default function TiposReceitaPage() {
  const router = useRouter()
  const [dados,   setDados]   = useState<TipoReceitaListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/cadastro/tipos-receita?${sp}`)
      if (!res.ok) { toast.error('Erro ao carregar'); return }
      const data: TipoReceitaListResponse = await res.json()
  ...
  async function toggleAtivo(t: TipoReceitaListItem) {
    if (!confirm(`${t.ativo ? 'Desativar' : 'Reativar'} "${t.descricao}"?`)) return
    const res = await fetch(`/api/cadastro/tipos-receita/${t.id}`, {
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>Classificação de receitas</div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/cadastro/tipos-receita/novo')}>
            ...
            <table className="table-base">
              <thead>
                <tr>
                  ...
                  <th style={{ width: 100 }}>Tipo Pai</th>
                  <th style={{ width: 80 }}>PIS/COF</th>
              ...
              <tbody>
                ...
                {!loading && dados.length === 0 && <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: 'var(--texto-terciario)' }}>Nenhum tipo de receita encontrado</td></tr>}
                {!loading && dados.map(t => (
                  <tr key={t.id}>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      {t.ind_pis_cofins
                        ? <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3, background: '#7C3AED20', color: '#7C3AED', fontWeight: 600 }}>Sim</span>
                        : <span style={{ fontSize: 11, color: 'var(--texto-terciario)' }}>—</span>}
                    </td>
                    <td><span className={`badge-status ${t.ativo ? 'badge-pago' : 'badge-cancelado'}`}>{t.ativo ? 'Ativo' : 'Inativo'}</span></td>
                    <td>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        <button onClick={() => router.push(`/cadastro/tipos-receita/${t.id}`)} className="btn-ghost" title="Editar" style={{ padding: '5px 8px' }}><Pencil size={13} /></button>
