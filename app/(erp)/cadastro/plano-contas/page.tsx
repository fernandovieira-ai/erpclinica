...
import type { PlanoContasListItem, PlanoContasListResponse } from '@/types/cadastros.types'

const CLASSIF_LABEL: Record<string, string> = {
  '01': 'Ativo', '02': 'Passivo', '03': 'Patrim. Líq.',
  '04': 'Resultado', '05': 'Compensação', '09': 'Outros',
}

export default function PlanoContasPage() {
  const router = useRouter()
  const [dados,   setDados]   = useState<PlanoContasListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const data: PlanoContasListResponse = await res.json()
      setDados(data.dados)
      setTotal(data.total)
      setPages(data.pages)
  ...
  async function toggleAtivo(c: PlanoContasListItem) {
    const acao = c.ativo ? 'desativar' : 'reativar'
    if (!confirm(`Deseja ${acao} "${c.descricao}"?`)) return
    const res = await fetch(`/api/cadastro/plano-contas/${c.id}`, {
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>Estrutura hierárquica contábil</div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/cadastro/plano-contas/novo')}>
          ...
          <div className="table-wrapper">
            <table className="table-base">
              ...
              <tbody>
                ...
                {!loading && dados.length === 0 && <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: 'var(--texto-terciario)' }}>Nenhuma conta encontrada</td></tr>}
                {!loading && dados.map(c => (
                  <tr key={c.id}>
                    <td style={{ fontFamily: 'var(--fonte-mono)', fontWeight: 600 }}>{c.codigo}</td>
                    <td>{c.descricao}</td>
                    <td style={{ fontSize: 12, color: 'var(--texto-terciario)' }}>{c.pai_desc ?? '—'}</td>
                    <td>
                      <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 3, fontWeight: 600,
                        ...
                        {c.tipo === 'S' ? 'Sintética' : 'Analítica'}
                      </span>
                    </td>
                    <td style={{ fontSize: 12 }}>{c.natureza === 'D' ? 'Devedora' : 'Credora'}</td>
                    <td style={{ fontSize: 12, color: 'var(--texto-secundario)' }}>{CLASSIF_LABEL[c.classificacao] ?? c.classificacao}</td>
                    <td><span className={`badge-status ${c.ativo ? 'badge-pago' : 'badge-cancelado'}`}>{c.ativo ? 'Ativa' : 'Inativa'}</span></td>
                    <td>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        <button onClick={() => router.push(`/cadastro/plano-contas/${c.id}`)} className="btn-ghost" title="Editar" style={{ padding: '5px 8px' }}><Pencil size={13} /></button>
