...
import type { TipoAtendimentoListItem, TipoAtendimentoListResponse } from '@/types/clinica.types'

export default function TiposAtendimentoPage() {
  const router = useRouter()
  const [dados,   setDados]   = useState<TipoAtendimentoListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/clinica/tipos-agendamento?${sp}`)
      if (!res.ok) { toast.error('Erro ao carregar'); return }
      const data: TipoAtendimentoListResponse = await res.json()
      setDados(data.dados)
      setTotal(data.total)
      setPages(data.pages)
  ...
  async function toggleAtivo(t: TipoAtendimentoListItem) {
    const acao = t.ativo ? 'desativar' : 'reativar'
    if (!confirm(`Deseja ${acao} "${t.descricao}"?`)) return
      ...
      <div className="page-header">
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>
            Tipos de atendimento utilizados nos agendamentos da clínica
          </div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/clinica/tipos-atendimento/novo')}>
            ...
            <table className="table-base">
              ...
              <tbody>
                ...
                {!loading && dados.map(t => (
                  <tr key={t.id}>
                    <td>
                      <div style={{ width: 16, height: 16, borderRadius: 3, backgroundColor: t.cor, margin: '0 auto' }} title={t.cor} />
                    </td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        <button onClick={() => router.push(`/clinica/tipos-atendimento/${t.id}`)} className="btn-ghost" title="Editar" style={{ padding: '5px 8px' }}>
                          <Pencil size={13} />
                        </button>
                        <button onClick={() => toggleAtivo(t)} className="btn-ghost" title={t.ativo ? 'Desativar' : 'Reativar'}
                          style={{ padding: '5px 8px', color: t.ativo ? 'var(--cor-erro)' : 'var(--cor-sucesso)' }}>
                          <PowerOff size={13} />
                        </button>
                      </div>
                    </td>
