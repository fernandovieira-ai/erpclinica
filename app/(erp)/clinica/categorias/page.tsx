...
import type { CategoriaListItem, CategoriaListResponse } from '@/types/clinica.types'

export default function CategoriasPage() {
  const router = useRouter()
  const [dados,   setDados]   = useState<CategoriaListItem[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/clinica/categorias?${sp}`)
      if (!res.ok) { toast.error('Erro ao carregar'); return }
      const data: CategoriaListResponse = await res.json()
      setDados(data.dados)
      setTotal(data.total)
      setPages(data.pages)
  ...
  async function toggleAtivo(c: CategoriaListItem) {
    const acao = c.ativo ? 'desativar' : 'reativar'
    if (!confirm(`Deseja ${acao} "${c.descricao}"?`)) return
    const res = await fetch(`/api/clinica/categorias/${c.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ativo: !c.ativo }),
    })
    if (res.ok) { toast.success(`Categoria ${c.ativo ? 'desativada' : 'reativada'}!`); carregar() }
      ...
      <div className="page-header">
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--texto-terciario)', marginTop: 2 }}>
            Categorias de atendimento (convênio, particular etc.) para os agendamentos
          </div>
        </div>
        <button className="btn-primary" onClick={() => router.push('/clinica/categorias/novo')}>
        ...
        <div className="card">
          <div className="table-wrapper">
            <table className="table-base">
              ...
              <tbody>
                {loading && (
                  <tr><td colSpan={3} style={{ textAlign: 'center', padding: 32, color: 'var(--texto-terciario)' }}>Carregando...</td></tr>
                )}
                {!loading && dados.length === 0 && (
                  <tr><td colSpan={3} style={{ textAlign: 'center', padding: 40, color: 'var(--texto-terciario)' }}>Nenhuma categoria encontrada</td></tr>
                )}
                {!loading && dados.map(c => (
                  <tr key={c.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        <button onClick={() => router.push(`/clinica/categorias/${c.id}`)} className="btn-ghost" title="Editar" style={{ padding: '5px 8px' }}>
                          <Pencil size={13} />
                        </button>
