...
// GET /api/cadastro/tipos-receita?busca=&ativo=true&page=1&limit=50
export async function GET(req: NextRequest) {
  ...
  const conds: string[]   = [`tr.empresa_id = $1`]
  const params: unknown[] = [session.empresa_id_ativa]
  let pi = 2

  if (ativo !== 'all') {
    conds.push(`tr.ativo = $${pi++}`)
    params.push(ativo !== 'false')
  }
  if (busca) {
    conds.push(`(tr.codigo ILIKE $${pi} OR tr.descricao ILIKE $${pi})`)
    params.push(`%${busca}%`)
    pi++
  }
  ...
  const [{ rows: countRows }, { rows }] = await Promise.all([
    db.query(`SELECT COUNT(*) AS n FROM tab_tipo_receita tr ${where}`, params),
    db.query(
      `SELECT tr.id, tr.codigo, tr.descricao, tr.natureza,
              pc.descricao AS conta_desc, pc.codigo AS conta_codigo,
              tr.ind_pis_cofins, tr.pai_id,
              p.descricao AS pai_desc, tr.ativo
...
// POST /api/cadastro/tipos-receita
export async function POST(req: NextRequest) {
  const session = await getSession(req)
  if (!session) return NextResponse.json({ erro: 'Não autenticado' }, { status: 401 })

  const body = tipoReceitaSchema.safeParse(await req.json())
  ...
  const { rows } = await db.query(
    `INSERT INTO tab_tipo_receita
       (empresa_id, codigo, descricao, natureza, conta_id, ind_pis_cofins, pai_id, ativo)
